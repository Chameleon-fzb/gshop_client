		...
		<div class="order-content">
			...
			<div class="orders">
				<table class="order-item" v-for="record in recordList" :key="record.id">
					...
					<tbody>
						<tr
							v-for="(orderGoodsItem, index) in record.orderDetailList"
							:key="orderGoodsItem.id"
						>
							<td width="60%">
								<div class="typographic">
									...
									<span>×{{ orderGoodsItem.skuNum }}</span>
									<a href="#" class="service">售后申请</a>
								</div>
							</td>
							<template v-if="index === 0">
								<td
									:rowspan="record.orderDetailList.length || 0"
									width="8%"
									class="center"
								>
									{{ record.consignee }}
								...
								>
									<ul class="unstyled">
										...
										<li>
											{{ record.paymentWay === 'ONLINE' ? '在线支付' : '到付' }}
										</li>
									</ul>
								</td>
								<td
...
export default {
	...
	methods: {
		async getMyOrderInfo(page = 1) {
			let { limit } = this
			try {
				await this.$store.dispatch('getMyOrderInfo', {
					page,
					limit
				})
			} catch (error) {
